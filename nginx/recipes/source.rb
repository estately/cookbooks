...
if node[:nginx][:tcp_proxy_module]
  ...
  git "/usr/src/nginx_tcp_proxy_module" do
    repository "https://github.com/yaoweibin/nginx_tcp_proxy_module.git"
    reference "master"
    action :checkout
  end

  node[:nginx][:configure_flags] << "--add-module=/usr/src/nginx_tcp_proxy_module"
end
